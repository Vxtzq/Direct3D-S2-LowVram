...
class SparseDownsample(nn.Module):
    ...
    def forward(self, input: SparseTensor) -> SparseTensor:
        ...
        OFFSET = torch.cumprod(torch.tensor(MAX[::-1], device=input.device), 0).tolist()[::-1] + [1]
        code = sum([c * o for c, o in zip(coord, OFFSET)])
        code, idx = code.unique(return_inverse=True)

        # safer accumulation: FP32 if input is FP16, else keep input dtype
        accum_dtype = torch.float32 if input.feats.dtype == torch.float16 else input.feats.dtype
        new_feats = torch.scatter_reduce(
            ...
            src=input.feats.to(accum_dtype),
            reduce=self.mode,
        )
        # cast back to original dtype (fp16 safe)
        new_feats = new_feats.to(input.feats.dtype)
...
class SparseUpsample(nn.Module):
    ...
    def forward(self, input: SparseTensor) -> SparseTensor:
        ...
        new_feats = input.feats[idx]  # inherits dtype (fp16-friendly)
...
class SparseSubdivide(nn.Module):
    """
    Subdivide a sparse tensor by a factor of 2 along each axis.
    """
    def __init__(self):
        super().__init__()
